...
- `0x09` 心跳
  - 不改变任何目标值，仅用于刷新 STM32 侧 ROS 在线判定
  - 建议 ROS 在无运动命令期间周期发送，周期应明显小于 500 ms
...
- Status_Task（20ms）
  - 每周期按预算轮询 4 轴状态/位置/速度，当前实现每轮最多发起 12 个查询（4 轴 x 状态/位置/速度）
  - 上报 ROS 状态帧（0x101）
...
- 子帧 `type=0x06`
  ...
  - byte2: reason/status
  - `0x80`: ROS 心跳超时
  - 其他值: 驱动返回的 status_flags
  - byte3~6: 故障发生时速度（int32，单位 0.1 RPM）

厂家 0x3A 状态位当前按如下规则分类：
...
因此 STM32 仅在 `0x08` 或 `0x80` 置位时触发紧急停机事件；限位、到位、使能、堵转预警会保留在状态字中供上位机观察，但不会单独触发急停。
...
- type=0x03
  - can1_tx_fail_count（CAN1 发报失败数，16bit）
  - can2_tx_fail_count（CAN2 发报失败数，16bit）
  - stop_fail_count（停机命令发送失败数，16bit）
